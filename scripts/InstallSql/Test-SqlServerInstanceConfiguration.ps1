...
param(
    ...
    [Parameter()]
    [ValidateNotNullOrEmpty()]
    [string]$LogRoot = 'C:\autoscript\logs\SqlInstanceCheck'
)
...
function Start-CheckTranscript {
    ...
    $transcriptPath = Join-Path -Path $Path -ChildPath "Test-SqlServerInstanceConfiguration-$safeInstance-$timestamp.log"
...
function Get-SqlCmdPath {
    ...
    Write-Warning 'sqlcmd.exe was not found. SQL Server instance check cannot continue.'
    ...
    Write-Host "Example: C:\AutoScript\scripts\InstallSql\Install-Ssms.ps1 -InstallerPath 'C:\install\SSMS-Setup-ENU.exe'" -ForegroundColor Yellow
    return $null
}
...
function Test-InstanceConfiguration {
    ...
    $physicalMemoryMb = [int64]$Current.PhysicalMemoryMb
    ...
    $targetMaxDop = Get-TargetMaxDop -CpuCoreCount $cpuCount

    return @(
        (New-CheckResult `
            -Item 'Enterprise Core version' `
        ...
        (New-CheckResult `
            ...
            -Passed ([int]$Current.MaxServerMemoryMb -eq $targetMaxMemoryMb) `
            -Expected "75% of total memory, minimum 2048 MB. Target=$targetMaxMemoryMb MB" `
            -Actual "$($Current.MaxServerMemoryMb) MB"),
        (New-CheckResult `
            -Item 'NUMA scheduler core count' `
        ...
        (New-CheckResult `
            -Item 'MAXDOP' `
            -Passed ([int]$Current.MaxDop -eq $targetMaxDop) `
            -Expected "CPU <=8: 2; <=16: 4; >=17: 8. Target=$targetMaxDop" `
...
try {
    $logPath = Start-CheckTranscript -Path $LogRoot -Instance $ServerInstance
    $transcriptStarted = $true
    Write-Host "Instance check log: $logPath" -ForegroundColor Cyan
    ...
    $current = Get-InstanceConfiguration
    $results = Test-InstanceConfiguration -Current $current
    ...
    if ($transcriptStarted) {
        ...
        Write-Host "Instance check log saved to: $logPath"
    }
}
