...
function Write-SqlInstanceExistsHelp {
    ...
    Write-Host 'C:\AutoScript\scripts\InstallSql\Install-SqlServer.ps1 -InstanceName SQL2019DEV'
    Write-Host ''
    Write-Host 'Example with separate data folders:'
    Write-Host 'C:\AutoScript\scripts\InstallSql\Install-SqlServer.ps1 -InstanceName SQL2019DEV -UserDbDir T:\SQLServerData_SQL2019DEV -UserDbLogDir T:\SQLServerLog_SQL2019DEV -TempDbDir T:\SQLServerTempDB_SQL2019DEV -BackupDir T:\SQLServerBackup_SQL2019DEV'
    Write-Host ''
}
...
function Write-SqlIsoMissingHelp {
    ...
    if (Test-Path -LiteralPath $isoFolder -PathType Container) {
        ...
        if ($null -ne $isoFiles) {
            ...
            Write-Host "C:\AutoScript\scripts\InstallSql\Install-SqlServer.ps1 -IsoPath '$($isoFiles[0].FullName)'"
            Write-Host "C:\AutoScript\scripts\InstallSql\Install-SqlServer.ps1 -IsoPath '$($isoFiles[0].FullName)' -InstanceName SQL2019DEV"
            Write-Host ''
            return
        }
    }
    ...
    Write-Host ''
    Write-Host 'Example:'
    Write-Host "C:\AutoScript\scripts\InstallSql\Install-SqlServer.ps1 -IsoPath 'D:\ISO\SQLServer2019-x64-ENU.iso'"
    Write-Host ''
}
