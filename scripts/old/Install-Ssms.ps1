...
function Write-SsmsInstallerMissingHelp {
    ...
    if (Test-Path -LiteralPath $installerFolder -PathType Container) {
        ...
        if ($null -ne $installerFiles) {
            ...
            Write-Host "C:\AutoScript\scripts\Install-Ssms.ps1 -InstallerPath '$($installerFiles[0].FullName)'"
            Write-Host "C:\AutoScript\scripts\Install-Ssms.ps1 -InstallerPath '$($installerFiles[0].FullName)' -InstallPath 'E:\SSMS'"
            Write-Host ''
            return
        }
    }
    ...
    Write-Host ''
    Write-Host 'Example:'
    Write-Host "C:\AutoScript\scripts\Install-Ssms.ps1 -InstallerPath 'D:\install\SSMS-Setup-ENU.exe'"
    Write-Host ''
}
